...
export default function CallPage() {
  ...
  const cleanupCall = (reason) => {
    ...
    console.log("cleanupCall called:", reason);
  ...
  return (
    <div className="page">
      ...
      <div className="video-container">
        ...
        {/* Resize / swap button */}
        {remoteStream && (
          <button className="resize-button" onClick={() => setIsLocalExpanded((prev) => !prev)}>
            ⛶
          </button>
        )}
      </div>
      ...
      <h2 className="status-text">
        {status === "idle" && "Choose a role"}
        {status === "searching" && "Searching for a match..."}
        {status === "matched" && "Match found! Start the call."}
        {status === "connected" && "Connected"}
        {status === "ended" && sessionStats && (
          <SessionSummary duration={sessionStats.duration} endedBy={sessionStats.endedBy} />
        )}
      </h2>

      {(status === "idle" || status === "ended") && (
        <div className="role-buttons">
          <button onClick={joinAsSeeker} disabled={status !== "idle" && status !== "ended"}>
            Need Support
          </button>
